    ...
    private void UpdateTokenVision()
    {
               ...
               if (count < 64)
                {
                    ...
                    // Usa a frente do sprite como direção visual do cone.
                    Vector2 dir = t.transform.up;
        ...
        if (fogMaterial != null)
        {
            ...
            if (count > 0)
            {
                ...
                fogMaterial.SetVectorArray("_TokenDirections", tokenDirArray);
            }
        }
    }
        ...
        for (int py = Mathf.Max(0, cy - r); py <= Mathf.Min(board.fogTex.height - 1, cy + r); py++)
        {
            for (int px = Mathf.Max(0, cx - r); px <= Mathf.Min(board.fogTex.width - 1, cx + r); px++)
            {
                ...
                else if (shape == VisionShape.Cone) 
                {
                    ...
                    else if (sqrDist == 0) inVision = true;
                }
                ...
                if (inVision)
                {
                    ...
                    {
                        board.fogPixels[idx] = new Color32(255, 255, 255, 128);
                        board.fogDirty = true;
                    }
                }
            }
