...
public class GMUIController : MonoBehaviour
{
    ...
    private float DrawPlayerScreenSection(RectTransform p, float y)
    {
        ...
        Button linkBtn = VTTLayout.BtnFixed(p, PAD, y, hw, BH - 4f, "VINCULAR CAMERAS", VTTLayout.C_BTN_SEC, VTTLayout.C_BDR_DEFAULT, VTTLayout.C_TEXT, 9f);
    ...
    {
        Button xrayBtn = VTTLayout.BtnFull(p, y, BH - 4f, -PAD * 2f, "VISAO RAIO-X: DESLIGADA", VTTLayout.C_BTN_SEC, VTTLayout.C_BDR_DEFAULT, Color.white, VTTLayout.F_BTN, bold: false);
        xrayBtn.onClick.AddListener(() => {
            if (fogController != null)
            {
                bool isXRay = !fogController.isXRayActive;
                fogController.ToggleXRay(isXRay);
                xrayBtn.GetComponentInChildren<TMP_Text>().text = isXRay ? "VISAO RAIO-X: LIGADA" : "VISAO RAIO-X: DESLIGADA";
    ...
    {
        VTTLayout.Label(p, y, 16f, VTTLayout.F_SMALL, VTTLayout.C_TEXT_DIM, FontStyles.Bold).text = "COR DA NEVOA";
        ...
        VTTLayout.Label(p, y, 16f, VTTLayout.F_SMALL, VTTLayout.C_TEXT_DIM, FontStyles.Bold).text = "OPACIDADE: AREA EXPLORADA";
        y -= 20f;
